...
fun applySukiSuCompatFixups() {
    ...
    // 4) The current build log's fatal symbol is ksu_handle_post_execveat_sucompat.
    // If the bridge references it but the manager's sucompat.c does not provide
    // it, add a source-compatible no-op post hook. The actual pre-exec sucompat
    // transformation remains in ksu_handle_execveat_sucompat(); this prevents
    // an ABI/linker mismatch from breaking vmlinux.
    if (sucompat.isFile &&
        bridge.readText().contains("ksu_handle_post_execveat_sucompat") &&
        !sucompat.readText().contains("ksu_handle_post_execveat_sucompat(")) {

        sucompat.appendText(
            """

            /*
             * SukiSU bridge compatibility:
             * some syscall_event_bridge revisions emit a post-exec sucompat
             * callback while builtin sucompat.c only implements the pre-exec
             * handler. Keep the callback ABI available without changing the
             * exec result or duplicating the pre-exec transformation.
             */
            int ksu_handle_post_execveat_sucompat(int *fd,
        ...
        logPostfix(sucompat, "injected missing ksu_handle_post_execveat_sucompat() compatibility implementation")
    }
...
when (mode) {
    "apply" -> apply()
    "postfix" -> postfix()
    "compat" -> applySukiSuCompatFixups()
    "revert" -> revert()
    else -> {

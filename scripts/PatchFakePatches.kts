...
fun findKsuRoot(): File? {
    ...
    // setup.sh installs KernelSU as drivers/kernelsu -> ../../KernelSU/kernel.
    // Resolve the symlink before checking feature/sucompat.c; otherwise a
    // relative/broken-looking symlink can make File(...).isFile return false
    // even though the real KernelSU source is present.
    val direct = f("drivers/kernelsu")
    if (direct.exists() || direct.isDirectory) {
        val resolved = try { direct.canonicalFile } catch (_: Exception) { direct }
        candidates += resolved
    }

    // Also search the source checkout itself.  Do not depend on the exact
    // KernelSU directory name because setup.sh may use a symlink.
    File(workDir).walkTopDown()
        .filter { it.isFile && it.name == "sucompat.c" && it.parentFile.name == "feature" }
        .forEach { candidates += it.parentFile.parentFile }

    File(workDir).walkTopDown()
        .filter { it.isFile && it.name == "syscall_event_bridge.c" }
        .forEach { candidates += it.parentFile.parentFile }

    return candidates
        .map { try { it.canonicalFile } catch (_: Exception) { it } }
        .distinctBy { it.path }
        .firstOrNull { File(it, "feature/sucompat.c").isFile }
}

fun applySukiSuCompatFixups() {
    /*
     * SUSFS may add the legacy/manual post-exec hook to fs/exec.c even when
     * the selected SukiSU-Ultra builtin tree no longer provides that symbol.
     * The old implementation only checked syscall_event_bridge.c, which made
     * this fix silently miss the exact failure seen on Android 14/6.1.
     */
    val ksuRoot = findKsuRoot()
    if (ksuRoot == null) {
        println("[compat] KernelSU tree not found; no SukiSU linker fixups needed")
        return
    }

    val bridge = File(ksuRoot, "hook/syscall_event_bridge.c")
    val sucompat = File(ksuRoot, "feature/sucompat.c")
    val setuidHook = File(ksuRoot, "hook/setuid_hook.c")

    if (!sucompat.isFile) {
        println("[compat] feature/sucompat.c not found; skipping SukiSU compatibility fixups")
        return
    }

    // 1) KernelSU/SukiSU builtin v4.x may expose the 3-argument setresuid
    // handler while an older bridge still calls the 2-argument API.
    if (bridge.isFile && bridge.readText().contains("ksu_handle_setresuid(old_uid, current_uid().val)")) {
        val has3Arg = setuidHook.isFile &&
            Regex("""(?:int|void)\s+ksu_handle_setresuid\s*\(\s*uid_t\s+ruid\s*,\s*uid_t\s+euid\s*,\s*uid_t\s+suid\s*\)""")
                .containsMatchIn(setuidHook.readText())

        if (has3Arg) {
            bridge.replaceEachLine(
                Regex("""ksu_handle_setresuid\(old_uid, current_uid\(\)\.val\);"""),
                """{
    ...
    ksu_handle_setresuid(ruid, euid, suid);
            ...
            logPostfix(bridge, "migrated legacy setresuid bridge call to the 3-argument SukiSU API")
        } else {
            logPostfix(bridge, "legacy setresuid bridge call found, but the 3-argument implementation was not detected; left unchanged")
        }
    }

    // 2) The tracepoint bridge has a different stat argument type from the
    // VFS stat hook. Keep the bridge-specific helper name.
    if (bridge.isFile && bridge.readText().contains("ksu_handle_stat(dfd, filename_user, flags);")) {
        bridge.replaceEachLine(
            Regex("""ksu_handle_stat\(dfd, filename_user, flags\);"""),
            "ksu_handle_stat_user(dfd, filename_user, flags);"
        )
        logPostfix(bridge, "renamed bridge stat call to ksu_handle_stat_user()")
    }

    // 3) If the bridge needs ksu_handle_stat_user(), provide it only when the
    // actual sucompat implementation does not already contain it.
    if (bridge.isFile &&
        bridge.readText().contains("ksu_handle_stat_user(") &&
        !sucompat.readText().contains("ksu_handle_stat_user(")) {

        sucompat.appendText(
            """

            /*
             * Compatibility helper for syscall_event_bridge.c.
             * This is intentionally separate from ksu_handle_stat(), whose
             * signature is used by the VFS hook path.
             */
            int ksu_handle_stat_user(int *dfd,
        ...
        logPostfix(sucompat, "injected missing ksu_handle_stat_user() bridge helper")
    }

    /*
     * 4) SUSFS/manual-hook compatibility for Android 14/6.1.
     *
     * The failing build references this symbol from fs/exec.c:
     *   ksu_handle_post_execveat_sucompat
     *
     * ReSukiSU's manual exec hook uses the six-argument ABI below.  SukiSU
     * Ultra's current builtin sucompat implementation does not provide that
     * legacy post hook, so a source-compatible no-op is sufficient: the
     * actual su->ksud/root transformation is already performed by
     * ksu_handle_execveat_sucompat() before exec.
     *
     * IMPORTANT: detect the reference in the whole kernel tree, not only in
     * syscall_event_bridge.c.  The previous implementation missed this case.
     */
    val commonRoot = File(workDir)
    val execC = f("fs/exec.c")
    val postHookReferenced =
        (execC.isFile && execC.readText().contains("ksu_handle_post_execveat_sucompat")) ||
        commonRoot.walkTopDown()
            .filter { it.isFile && it.extension in setOf("c", "h") }
            .any { it.readText().contains("ksu_handle_post_execveat_sucompat") }

    val postHookDefinition = Regex(
        """\bksu_handle_post_execveat_sucompat\s*\(\s*int\s*\*\s*fd\s*,\s*struct\s+filename\s*\*\*\s*filename_ptr\s*,\s*void\s*\*\s*argv\s*,\s*void\s*\*\s*envp\s*,\s*int\s*\*\s*flags\s*,\s*int\s*\*\s*retval\s*\)"""
    )

    if (postHookReferenced && !postHookDefinition.containsMatchIn(sucompat.readText())) {
        sucompat.appendText(
            """

            /*
             * SukiSU/SUSFS compatibility:
             * Some SUSFS/manual-hook revisions add a post-exec callback to
             * fs/exec.c, while current SukiSU-Ultra sucompat.c no longer
             * implements the legacy callback. The real sucompat work is done
             * by ksu_handle_execveat_sucompat() before exec; keep the legacy
             * callback as a no-op so the kernel links cleanly.
             */
        ...
        logPostfix(
            sucompat,
            "injected missing ksu_handle_post_execveat_sucompat() six-argument compatibility stub"
        )
    } else if (postHookReferenced) {
        logPostfix(
            sucompat,
            "ksu_handle_post_execveat_sucompat() already provided; no linker stub needed"
        )
    }

    println("[compat] SukiSU bridge compatibility scan complete")
}
fun postfix() {

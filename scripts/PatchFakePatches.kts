...
fun applySukiSuCompatFixups() {
     ...
     *
     * Do NOT require feature/sucompat.c to exist. Newer SukiSU trees may
     * reorganize/remove that file. The symbol only needs one real compiled
     * definition. We place a tiny compatibility implementation in the
     * always-built drivers/kernelsu/ksu.c when the symbol is otherwise absent.
     */
    ...
    val ksuRoot = try { ksuLink.canonicalFile } catch (_: Exception) { ksuLink }
    if (!ksuRoot.exists() || !ksuRoot.isDirectory) {
        println("[compat] drivers/kernelsu could not be resolved; cannot install post-exec compatibility symbol")
        return
    }
    ...
    /*
     * ksu.c is part of drivers/kernelsu and is compiled by the normal
     * KernelSU Makefile. The arguments intentionally use opaque pointers so
     * this compatibility shim does not depend on a particular exec API.
     */
    val ksuC = File(ksuRoot, "ksu.c")
    if (!ksuC.isFile) {
        println("[compat] drivers/kernelsu/ksu.c was not found; cannot install compatibility symbol")
        return
    }
    ...
    ksuC.appendText(
         ...
         * Some SUSFS/manual-hook revisions still call this callback from
         * fs/exec.c, while newer SukiSU trees no longer provide it.
         * The actual sucompat handling occurs before exec, so the legacy
         * callback is intentionally a no-op. Its presence is required only
         * to satisfy the kernel linker on this older hook path.
         */
    ...
    logPostfix(ksuC, "injected legacy ksu_handle_post_execveat_sucompat() compatibility shim because fs/exec.c references it")
}

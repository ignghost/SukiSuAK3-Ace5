...
fun findKsuRoot(): File? {
    ...
    return candidates.distinct().firstOrNull { File(it, "hook/syscall_event_bridge.c").isFile }
}

fun applySukiSuCompatFixups() {
    // 4) The bridge currently references ksu_handle_post_execveat_sucompat,
    // but some SukiSU/KernelSU trees do not compile a definition into vmlinux.
    // Put a weak ABI-compatible fallback in fs/exec.c itself. Because this
    // symbol is then emitted by fs/exec.o (the object reporting the failure),
    // the vmlinux link cannot fail due to an absent archive member. A real
    // strong KernelSU implementation, if present, overrides this weak one.
    val execC = f("fs/exec.c")
    if (execC.isFile &&
        bridge.readText().contains("ksu_handle_post_execveat_sucompat") &&
        ...
        execC.appendText(
            ...
            /*
             * SukiSU bridge linker compatibility fallback.
             *
             * Some SukiSU bridge revisions call the post-exec sucompat hook
             * while the selected KernelSU tree does not export the function
             * into vmlinux. Keep the ABI available from fs/exec.o.
             *
             * __weak allows a real KernelSU implementation to override this
             * fallback when one is actually linked.
             */
            int __weak ksu_handle_post_execveat_sucompat(
                int *fd,
        ...
        logPostfix(execC, "injected weak ksu_handle_post_execveat_sucompat() fallback directly into fs/exec.c")
    } else if (execC.isFile && execC.readText().contains("ksu_handle_post_execveat_sucompat(")) {
        logPostfix(execC, "fs/exec.c already contains ksu_handle_post_execveat_sucompat(); no fallback added")
    }
    ...
    if (bridge.readText().contains("ksu_handle_setresuid(old_uid, current_uid().val)")) {
    ...
    if (bridge.readText().contains("ksu_handle_stat(dfd, filename_user, flags);")) {
        bridge.replaceEachLine(

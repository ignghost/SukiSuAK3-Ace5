...
fun applySukiSuCompatFixups() {
    ...
    // 4) The fatal linker symbol is ksu_handle_post_execveat_sucompat.
    // The bridge can reference it even when the selected KernelSU sucompat
    // object is not linked into vmlinux. Put a weak fallback directly into
    // fs/exec.c, because the linker failure identifies fs/exec.o as the
    // unresolved caller/object path. A real strong implementation, if linked,
    // overrides this weak fallback.
    if (bridge.readText().contains("ksu_handle_post_execveat_sucompat")) {
        val exec = f("fs/exec.c")
        if (exec.isFile &&
            !exec.readText().contains("int __weak ksu_handle_post_execveat_sucompat")) {
            ...
            exec.appendText(
                 ...
                 *
                 * Some SukiSU bridge revisions call the post-exec sucompat
                 * hook while the selected KernelSU tree does not export the
                 * function into vmlinux. Keep the ABI available from fs/exec.o.
                 *
                 * __weak allows a real KernelSU implementation to override
                 * this fallback when one is actually linked.
                 */
...
"
            ...
            logPostfix(exec, "injected weak ksu_handle_post_execveat_sucompat() fallback directly into fs/exec.c")
        } else if (exec.isFile) {
            logPostfix(exec, "weak ksu_handle_post_execveat_sucompat() fallback already present in fs/exec.c")
        }
    }

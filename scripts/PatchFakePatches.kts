...
fun findKsuRoot(): File? {
    ...
    if (direct.isDirectory) candidates += direct

    // Prefer a real SukiSU/KernelSU root identified by sucompat.c.  The
    // post-exec SUSFS failure can occur even when syscall_event_bridge.c is
    // absent or has already moved in the selected builtin tree.
    ...
    return candidates.distinct().firstOrNull { File(it, "feature/sucompat.c").isFile }
}

...
/**
 * Ensure compatibility with older SUSFS manual exec hooks.
 *
 * Some SUSFS revisions add a legacy six-argument post-exec callback to
 * fs/exec.c. Modern SukiSU trees do not always provide that callback because
 * their exec handling is performed by the pre-exec execveat hook.
 *
 * Keep the legacy callback as a harmless compatibility shim. Do this after
 * SUSFS has patched fs/exec.c, so the decision is based on the actual source
 * that will be compiled. Never key this on a particular KernelSU layout.
 */
fun ensureLegacyPostExecSucompat() {
    val exec = f("fs/exec.c")
    if (!exec.exists()) return

    val execText = exec.readText()
    val symbol = "ksu_handle_post_execveat_sucompat"
    if (!execText.contains(symbol)) {
        return
    }

    // drivers/kernelsu is normally a symlink created by KernelSU setup.sh.
    val ksuLink = f("drivers/kernelsu")
    val candidates = mutableListOf<File>()

    if (ksuLink.exists()) {
        val resolved = try {
            ksuLink.canonicalFile
        } catch (_: Exception) {
            null
        }
        if (resolved != null) {
            candidates += File(resolved, "feature/sucompat.c")
            candidates += File(resolved, "kernel/feature/sucompat.c")
        }
    }

    // Fallback for trees where the KernelSU directory is not exposed through
    // drivers/kernelsu or the symlink is unavailable.
    candidates += f("drivers/kernelsu/feature/sucompat.c")
    candidates += f("KernelSU/kernel/feature/sucompat.c")

    var sucompat: File? = candidates.firstOrNull { it.isFile }

    if (sucompat == null) {
        val roots = listOf(f("drivers/kernelsu"), f("KernelSU"))
            .filter { it.exists() }
        for (root in roots) {
            sucompat = root.walkTopDown()
                .firstOrNull { it.isFile && it.name == "sucompat.c" }
            if (sucompat != null) break
        }
    }

    if (sucompat == null) {
        println("[postfix]-fs/exec.c: ERROR: legacy $symbol reference detected but no sucompat.c could be located")
        return
    }
    ...
    // A declaration/prototype alone is not a definition. Detect the function
    // body specifically so an extern declaration does not satisfy this check.
    val definition = Regex(
        """(?m)^\s*(?:static\s+)?(?:__attribute__\s*\(\([^)]*\)\)\s*)?int\s+$symbol\s*\("""
    )
    if (definition.containsMatchIn(source)) {
        println("[postfix]-${file.rel()}: legacy $symbol definition already present")
        return
    }

    val shim = """
/*
 * SUSFS legacy compatibility shim.
 *
 * Older SUSFS manual hooks call this post-exec callback. SukiSU's current
 * execveat path performs its su-compat handling before the real exec, so the
 * legacy callback does not need to alter the exec arguments here.
 */
int $symbol(int *fd,
...
""".trimIndent()
    ...
    file.writeText(source.trimEnd() + "\n\n" + shim + "\n")
    println("[postfix]-${file.rel()}: injected six-argument legacy $symbol compatibility shim")
}

fun postfix() {
    applyPostPatchFixups()
    ensureLegacyPostExecSucompat()
}

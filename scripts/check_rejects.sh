...
check_ksu_compat_symbols() {
  ...
  # The symbol may be in feature/sucompat.c, another sucompat.c, or ksu.c.
  # Only the real compiled definition matters.
  local SYMBOL_FILE
  ...
  if [ -z "$SYMBOL_FILE" ]; then
    echo "::error title=SukiSU compatibility::fs/exec.c references ksu_handle_post_execveat_sucompat but no KernelSU C file defines it"

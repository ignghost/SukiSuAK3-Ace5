...
check_ksu_compat_symbols() {
  ...
  # KernelSU is normally exposed as drivers/kernelsu -> KernelSU/kernel.
  # Resolve the symlink before falling back to a tree-wide search.
  local KSU_LINK="$SEARCH_DIR/drivers/kernelsu"
  if [ -e "$KSU_LINK" ]; then
    local KSU_ROOT
    KSU_ROOT=$(readlink -f "$KSU_LINK" 2>/dev/null || true)
    if [ -n "$KSU_ROOT" ] && [ -f "$KSU_ROOT/feature/sucompat.c" ]; then
      SUCOMPAT_REF="$KSU_ROOT/feature/sucompat.c"
    fi
  fi

  if [ -z "$SUCOMPAT_REF" ]; then
    SUCOMPAT_REF=$(find -L "$SEARCH_DIR" -type f -path '*/feature/sucompat.c' -print -quit 2>/dev/null || true)
  fi
